...
public class CameraTargetOrientationScript : MonoBehaviour
{
    ...
    void OnEnable()
    {
      ...
      //  Debug.Log("Cam Offset: "+ _cameraOffset);
    ...
    void LateUpdate()
    {
        ...
        if (enableRotation && (Input.touchCount == 1))
        {
            ...
            //Quaternion camAngle = Quaternion.AngleAxis(Input.GetTouch(0).deltaPosition.x * rotationSpeedTouch, Vector3.up);
            Vector3 newPos = target.position + _cameraOffset;
            _cameraOffset = camAngle * _cameraOffset;
        ...
        else
        {
            ...
            if (Input.touchCount == 2)
            {
                ...
                this.GetComponent<Camera>().fieldOfView = Mathf.Clamp(this.GetComponent<Camera>().fieldOfView, minZoom, maxZoom);
